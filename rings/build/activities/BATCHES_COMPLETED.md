...
**Tests:**
- [x] `SlowMockRunHandle::try_wait` returns `Ok(None)` for N calls then `Ok(Some(output))`
- [x] `stream_cost_entries` on a 1,000-line `costs.jsonl` returns correct entries without slurping
- [x] All existing tests in `engine_integration.rs` and `engine_timeout_cancel.rs` pass after refactor
...
**Tests:**
- [x] `stale_removed` is `Some(StaleLockInfo { run_id, pid })` when stale lock removed
- [x] `stale_removed` is `None` when no stale lock existed
- [x] Captured stderr contains run_id and PID when stale lock emits warning
- [x] Active PID lock still returns `LockError::ActiveProcess` (no regression)
- [x] `--force-lock` still bypasses lock check (no regression)
...
**Tests:**
- [x] All 9 known variables (`phase_name`, `cycle`, `max_cycles`, `iteration`, `runs_per_cycle`, `run`, `cost_so_far_usd`, `workflow_name`, `context_dir`) produce no warning
- [x] `{{typo}}` in `unknown_vars`; remains as literal `{{typo}}` in rendered output
- [x] `{{{{` → `{{` in rendered output; `unknown_vars` is empty
- [x] `{{{{phase_name}}}}` → `{{phase_name}}` in rendered output; NOT flagged as unknown
- [x] Two different unknown variables in same template: both reported
- [x] Same unknown variable appearing twice: reported only once (deduplication)
- [x] Unknown variables across multiple phases: all reported at startup
- [x] Integration: workflow with unknown variable emits advisory_warning but execution proceeds
...
- [x] JSONL mode: valid `dry_run_plan` event with fields `found`, `line_number`, `phase_name`
- [x] Integration: `rings run --dry-run workflow.toml` exits 0; no executor subprocess spawned (use mock that panics on spawn)
- [x] Integration: `rings run --dry-run invalid.toml` exits 2
...
**Tests:**
- [x] Valid `state.json` → `StateLoadResult::Ok`
- [x] Corrupt `state.json` + valid `costs.jsonl` with ≥1 entry → `Recovered` with run number in warning string
- [x] Corrupt `state.json` + empty `costs.jsonl` → `Unrecoverable`
- [x] Corrupt `state.json` + absent `costs.jsonl` → `Unrecoverable`
- [x] Both files corrupt → `Unrecoverable`
- [x] Integration: captured stderr contains both absolute file paths on `Unrecoverable`
- [x] Malformed JSONL lines skipped; max run from valid lines only
- [x] Ordering invariant: `costs.jsonl` has entry N, `state.json` records N-1 (crash simulated) → recovery returns N
- [x] `costs.jsonl` with one `cost_usd = null` entry followed by valid entry → recovery picks valid entry's run number
- [x] Integration: `rings resume` from corrupt state reconstructs position and continues
...
**Tests:**
- [x] Identical fingerprints → no error, no warning
- [x] Added phase → exit code 2, error message contains "phases not present"
- [x] Removed phase → exit code 2, error message contains "phases removed"
- [x] Reordered phases → exit code 2, error message contains "phase order has changed"
- [x] Changed `runs_per_cycle` only → exit 0 with exact non-structural warning text
- [x] Changed `max_cycles` only → exit 0 with non-structural warning text
- [x] Changed `completion_signal` → exit 0 with non-structural warning text
- [x] Changed prompt text only → exit 0 with non-structural warning text
- [x] Missing `phase_fingerprint` in old `run.toml` → skip check, advisory warning, proceed
- [x] Integration: captured stderr contains exact non-structural change warning text
- [x] Integration: `rings resume` with structurally changed workflow exits code 2
...
**Tests:**
- [x] `RunStatus` round-trip: each variant via `toml::to_string` / `toml::from_str`
- [x] Old `run.toml` with bare `status = "running"` deserializes correctly
- [x] `rings list --format jsonl` and `rings list --output-format jsonl` parse identically
- [x] `rings list --status bogus` exits non-zero and lists valid choices
- [x] `scan_runs` returns entries sorted by date descending
- [x] `--status` filter excludes non-matching runs
- [x] `--workflow` substring filter matches partial path
- [x] `--since 7d` excludes runs older than 7 days
- [x] `--since 2024-03-15` excludes runs before that date (midnight UTC)
- [x] Run at `2024-03-15T23:30:00-05:00` (UTC `2024-03-16T04:30:00Z`) included with `--since 2024-03-16` (UTC-day test)
- [x] `--since 2024-13-01` exits with clap error at parse time
- [x] `--since 0d` exits with error
- [x] `--limit N` truncates to N most recent entries
- [x] `--limit 5` on 50-run directory: exactly 5 `run.toml` files opened (early termination)
- [x] Empty base directory: returns empty list, no error
- [x] Unreadable run directory: silently skipped
- [x] Two runs with identical `started_at` second: sort is deterministic (stable by run_id tiebreaker)
- [x] `rings list --status incomplete` filters correctly (record spec gap in REVIEW.md)
- [x] Integration: JSONL mode emits one JSON object per run with correct fields
